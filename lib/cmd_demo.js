...
const PERSONAS = [
  {
    ...
    habits: {
      sessionsSampled: 74, promptsSampled: 812, budgetChars: 244000,
      summary: 'Ada opens big work with edges, not features: every epic starts by naming what is out of scope before what is in, then cuts the rest into phases that each end at a verification gate. The agent proves a phase (test run, screenshot) before the next begins, and acceptance criteria arrive as observable outcomes rather than adjectives. Bugs get the same discipline in miniature — repro command, observed symptom, suspected file — so the first agent turn is a fix attempt, not a search. Mechanical work is pushed down to subagents; anything irreversible stays in the main thread for review.',
      traits: [
        { title: 'Scopes out before scoping in', kind: 'scoping', strength: 3,
          detail: 'Epics open with an explicit in-scope / out-of-scope split, so the agent never wanders into next quarter\'s work.',
          evidence: [{ quote: 'Out of scope: per-site permissions (next quarter), mobile layout changes.' }] },
        { title: 'Phases with a verification gate each', kind: 'planning', strength: 3,
          detail: 'Large work is cut into reviewable phases, and the agent must prove each one before the next begins.',
          evidence: [{ quote: 'After each phase run the e2e suite and show me a screenshot before moving on.' }] },
        { title: 'Quality bars are observable, not adjectives', kind: 'standards', strength: 2,
          detail: 'Acceptance criteria are stated as things you could check — which makes "done" checkable too.',
          evidence: [{ quote: 'Quality bar: no visible layout shift when switching sites.' }] },
        { title: 'Anchors bugs to the exact repro', kind: 'verification', strength: 3,
          detail: 'Bug reports carry the repro command, the observable symptom and the suspected file.',
          evidence: [{ quote: 'repro: docker restart broker while watching /debug/subs — count goes to 0 and stays' }] },
        { title: 'Delegates the mechanical, keeps the risky', kind: 'delegation', strength: 2,
          detail: 'Subagents get renames and boilerplate; schema changes and anything irreversible stay under review.',
          evidence: [{ quote: 'Use subagents for the mechanical renames so the main thread stays focused on the schema changes.' }] },
      ],
      templates: [
        { title: 'Open an epic with edges', trait: 'Scopes out before scoping in',
          prompt: 'We\'re adding <feature> to <project>. Goal: <one-sentence outcome>.\n\nIn scope: <a, b, c>. Out of scope: <x (deferred to when)>, <y>.\n\nWork in phases — 1) <phase>, 2) <phase>, 3) <phase>. After each phase run <suite> and show me <proof> before moving on. Quality bar: <observable outcome>.' },
        { title: 'Bug report in four beats', trait: 'Anchors bugs to the exact repro',
          prompt: 'Fix <symptom> in <project>: when <trigger>, <observed behaviour> (repro: <command> while watching <where> — <what you see>). Expected: <behaviour, with a limit>. Suspect <file or path>.' },
        { title: 'Delegate with a stop condition', trait: 'Delegates the mechanical, keeps the risky',
          prompt: 'Plan first, then implement. Break <the work> into steps I can review; after each step run <tests> and stop if anything fails twice in a row. Use subagents for <the mechanical part>. Don\'t push anything — leave commits local for my review.' },
      ],
    },
  },
  {
    ...
    habits: {
      sessionsSampled: 58, promptsSampled: 517, budgetChars: 181000,
      summary: 'Grace writes short, dense asks that define "done" before any work starts — usually a countable condition plus a doc note, so the fix teaches the codebase something. She refuses the easy workaround by name (no retry bumps, no buffer-it-in-memory) and makes the agent root-cause instead. Research requests come with an output format and bounded depth, which turns "look into it" into a deliverable. Threads stay short: one task per session, re-briefed cheaply, with a quick nudge the moment the agent drifts from the constraint that matters.',
      traits: [
        { title: 'Defines "done" before the work starts', kind: 'standards', strength: 3,
          detail: 'Tasks end with a countable condition, not a feeling — and often a one-line doc note so the lesson sticks.',
          evidence: [{ quote: 'Done means: retries removed, suite green 20 consecutive runs (`npm run test:loop 20`), and a short note in TESTING.md about the timer rule.' }] },
        { title: 'Root-causes instead of papering over', kind: 'verification', strength: 3,
          detail: 'Names the tempting workaround and forbids it, so the agent digs for the shared underlying cause.',
          evidence: [{ quote: 'Root-cause each one instead of bumping retries — they share a leaked fake-timer pattern.' }] },
        { title: 'Research asks carry an output format', kind: 'scoping', strength: 2,
          detail: 'Comparisons specify the columns, the sources and the decision they feed, which bounds the depth automatically.',
          evidence: [{ quote: 'Output: a one-page table (throughput, replay, ops burden, our-stack fit) + your recommendation with the single strongest reason and the single biggest risk.' }] },
        { title: 'States the constraint that will bite', kind: 'standards', strength: 2,
          detail: 'Acceptance criteria include the resource limit or scale that would otherwise surface in production.',
          evidence: [{ quote: 'Accept: a 500k-row export completes without the node process exceeding 300MB RSS.' }] },
        { title: 'One task, one session', kind: 'session-management', strength: 2,
          detail: 'Starts fresh per task instead of dragging stale context; short threads keep the model on the current constraint.',
          evidence: [{ quote: 'new session for the export work — the flaky-test context doesn\'t apply here' }] },
      ],
      templates: [
        { title: 'Say what done means', trait: 'Defines "done" before the work starts',
          prompt: '<task> in <project> (search "<marker>" to find the spots). Root-cause instead of <the easy workaround>. Done means: <the workaround removed>, <suite> green <N> consecutive runs (`<command>`), and a short note in <doc> about <the rule>.' },
        { title: 'Research with a deliverable', trait: 'Research asks carry an output format',
          prompt: 'Before we pick <choice> for <project>: compare <option A> vs <option B> for our workload (<the numbers that matter>). Read the actual docs, not blog posts. Output: a one-page table (<columns>) + your recommendation with the single strongest reason and the single biggest risk.' },
        { title: 'Acceptance with a resource limit', trait: 'States the constraint that will bite',
          prompt: 'Add <feature> to <project>. Reuse <existing mechanism> so <it stays consistent>. <The perf approach — stream, don\'t buffer>. Accept: <the large case> completes without <process> exceeding <limit>.' },
      ],
    },
  },
];
...
async function run(args) {
  ...
  for (const p of PERSONAS) {
    ...
    writeJson(path.join(dir, 'habits.json'), {
      schema: 1, generatedAt: new Date().toISOString(), grader: p.tool, ...p.habits,
    });
