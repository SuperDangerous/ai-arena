...
async function run(args) {
  ...
  if (!files.length) {
    log('');
    log('No Claude Code or Codex session logs were found on this machine.');
    log('  Arena reads the history those CLIs write as you use them:');
    log('    Claude Code → https://claude.com/claude-code   (logs in ~/.claude/projects)');
    log('    Codex CLI   → https://developers.openai.com/codex/cli   (logs in ~/.codex/sessions)');
    log('  Use either for a while, then run this again. Logs somewhere unusual? node arena.js scan --add <path>');
  }
