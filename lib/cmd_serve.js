...
function buildDataset() {
  ...
  const body = JSON.stringify({
    ...
    me: cfg.slug,
    config: { user: cfg.user, include: cfg.include, excludes: cfg.excludes, sources: cfg.sources, dataDir: store.dataDir(), dataRepo: publishCore.isRepo() },
    pricing: loadTable(),
    git: gitStatus(),
...
async function run(args) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (p.startsWith('/data/') && /\.(png|jpe?g|webp|gif|svg)$/i.test(p)) {
      ...
      if (!file.startsWith(store.dataDir())) { res.writeHead(403); res.end(); return; }
    } else {
